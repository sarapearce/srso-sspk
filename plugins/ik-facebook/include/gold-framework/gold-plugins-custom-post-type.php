...
*/
if(!class_exists("WPSP_GoldPlugins_CustomPostType")):
class WPSP_GoldPlugins_CustomPostType
{
	...
	function setupCustomPostType($postType)
	{
		...
		$menu_icon = isset($postType['menu_icon']) ? $postType['menu_icon'] : NULL; //RWG: added to allow user ability to pass in a custom menu icon for post type, accetpable options here: https://developer.wordpress.org/resource/dashicons/
		...
		{		
			...
			$args = array(
				'labels' => $labels,
				'public' => !empty($postType['public']) ? $postType['public'] : false,
				'publicly_queryable' => !empty($postType['public']) ? $postType['public'] : false,
				'show_ui' => true, 
				'query_var' => true,
				...
				'menu_icon' => $menu_icon, //RWG: added to allow user ability to pass in a custom menu icon for post type, accetpable options here: https://developer.wordpress.org/resource/dashicons/
				//hide ability to create new posts
				'capabilities' => array(
					'create_posts' => 'do_not_allow', // phony role, so no one can use it (http://stackoverflow.com/a/16675677)
				),
				'map_meta_cap' => true, // required to still show/allow edit and other actions, even though create_posts is disabled
			); 
	...
	function setupCustomFields($fields)
	{
		...
		foreach ($fields as $f)
		{
			...
			(
				...
				"disabled"		=> !empty($f['disabled']) ? $f['disabled'] : false
			);
		}
	...
	function displayCustomFields() {
		...
		<div class="form-wrap">
			...
			foreach ( $this->customFields as $customField ) {
				...
				foreach ( $scope as $scopeItem ) {//RWG: 10.28.15 remove whitespace as custom fields weren't displaying on edit due to scope mismatch
					$scopeItem = str_replace(" ", "", $scopeItem);
				...
				if ( $output ) { ?>
					<div class="form-field form-required">
						<?php
						$disabled = $customField['disabled'] ? 'disabled="disabled"' : '';
						
						switch ( $customField[ 'type' ] ) {
							case "checkbox": {
								// Checkbox
								echo '<label for="' . $this->prefix . $customField[ 'name' ] .'" style="display:inline;"><b>' . $customField[ 'title' ] . '</b></label>&nbsp;&nbsp;';
								echo '<input '.$disabled.' type="checkbox" name="' . $this->prefix . $customField['name'] . '" id="' . $this->prefix . $customField['name'] . '" value="yes"';
							...
							case "textarea": {
								...
								echo '<textarea '.$disabled.' name="' . $this->prefix . $customField[ 'name' ] . '" id="' . $this->prefix . $customField[ 'name' ] . '" columns="30" rows="3">' . htmlspecialchars( get_post_meta( $post->ID, $this->prefix . $customField[ 'name' ], true ) ) . '</textarea>';
								break;
							}
							default: {
								// Plain text field
								echo '<label for="' . $this->prefix . $customField[ 'name' ] .'"><b>' . $customField[ 'title' ] . '</b></label>';
								echo '<input '.$disabled.' type="text" name="' . $this->prefix . $customField[ 'name' ] . '" id="' . $this->prefix . $customField[ 'name' ] . '" value="' . htmlspecialchars( get_post_meta( $post->ID, $this->prefix . $customField[ 'name' ], true ) ) . '" />';
								break;
							}
						}
						?>
